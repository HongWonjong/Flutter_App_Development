...
class SrtModifyState {
  final String? srtContent; // 원본 SRT
  ...
  final String? firstSubtitle; // 새로 추가된 필드: 첫 번째 자막 텍스트
  ...
  SrtModifyState({
    ...
    this.firstSubtitle, // 기본값은 null로 설정
  });
  ...
  SrtModifyState copyWith({
    ...
    String? errorMessage,
    bool? isModified,
    String? firstSubtitle, // copyWith에 추가
  }) {
    return SrtModifyState(
      ...
      firstSubtitle: firstSubtitle ?? this.firstSubtitle, // 기존 값 유지
    );
  }
}
...
class SrtModifyNotifier extends StateNotifier<SrtModifyState> {
  ...
  void setSrtContent(String srtContent) {
    final parsedEntries = SrtModifyService.parseSrt(srtContent);
    final firstSubtitle = _getFirstSubtitle(srtContent); // 첫 자막 추출
    state = state.copyWith(
      srtContent: srtContent,
      parsedSrtEntries: parsedEntries,
      modifiedSrtEntries: parsedEntries, // 초기에는 파싱된 항목으로 설정
      isModified: false,
      firstSubtitle: firstSubtitle, // 상태에 첫 자막 저장
    );
  }
  ...
  void updateModifiedEntries(List<SrtEntry> modifiedEntries) {
    ...
    state = state.copyWith(
      ...
      isModified: true,
      firstSubtitle: firstSubtitle, // 상태에 업데이트된 첫 자막 저장
    );
  }
